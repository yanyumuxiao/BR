...
# beam search {{{
def beam_search_decoding(decoder,  # 解码器
                         enc_outs,  # 编码器的输入特征，即序列特征
                         enc_last_h,  # 编码输出的最后一个隐状态
                         beam_width,  # beam width
                         n_best,  # 对于每一个输入序列，最好的输出个数， n_best = 5
                         sos_token,  # 起始符
                         eos_token,  # 结束符
                         max_dec_steps,  # 最大解码步数
                         device):
    # 返回n个解码最好的结果list
    ...
    for batch_id in range(bs):
        ...
        while True:
            ...
            num_class = decoder_output.shape[-1]
            mask_idx = [sos_token, eos_token]
            mask = torch.tensor([0 if i in mask_idx else 1 for i in range(num_class)]).cuda()
            decoder_output = F.softmax(decoder_output, dim=1)
            decoder_output *= mask
            # one_hot_sos = torch.zeros(1, num_class).long()
            # one_hot_eos = torch.zeros(1, num_class).long()
            # sos_index = torch.LongTensor([sos_token]).unsqueeze(dim=1)
            # eos_index = torch.LongTensor([eos_token]).unsqueeze(dim=1)
            # sos_mask = one_hot_sos.scatter_(dim=1, index=sos_index, src=torch.ones(1, num_class).long())
            # eos_mask = one_hot_eos.scatter_(dim=1, index=eos_index, src=torch.ones(1, num_class).long())
            # mask = sos_mask + eos_mask
